 ...
 * 
 * هذا الكود آمن 100%:
 * - ينشئ الجداول المفقودة فقط (IF NOT EXISTS).
 * - يضيف الأعمدة الناقصة للجداول الموجودة (ADD COLUMN IF NOT EXISTS).
...
ALTER TABLE public.office_users ADD COLUMN IF NOT EXISTS device_name TEXT;
ALTER TABLE public.office_users ADD COLUMN IF NOT EXISTS force_logout BOOLEAN DEFAULT FALSE;
