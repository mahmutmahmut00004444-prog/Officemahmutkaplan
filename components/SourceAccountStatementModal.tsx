...
const SourceAccountStatementModal: React.FC<SourceAccountStatementModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-[2000] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-scale-up" onClick={onClose}>
      ...
      <div className="bg-white p-6 rounded-[2.5rem] w-full max-w-5xl max-h-[90vh] flex flex-col border-2 border-slate-900 shadow-2xl relative overflow-hidden" onClick={e => e.stopPropagation()}>
        <div className="absolute top-0 right-0 z-10 p-4"><button onClick={onClose} className="p-3 bg-white text-slate-600 rounded-full shadow-lg border border-slate-100 hover:bg-slate-50 transition-all active:scale-95 flex items-center justify-center"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="m15 18-6-6 6-6"/></svg></button></div>
        <div className="pt-8 flex flex-col h-full">
            <h3 className="text-2xl font-black text-slate-900 text-right pr-2">كشف حساب: {source.sourceName}</h3>
            
            <div className="flex justify-center gap-2 my-6 p-2 bg-slate-50 rounded-xl border border-slate-200 overflow-x-auto shrink-0">
