...
const ContextMenuModal: React.FC<ContextMenuModalProps> = ({ isOpen, onClose, menuItems, title, description, warningMessage }) => {
  ...
  return (
    ...
    >
      <div 
        ref={modalRef}
        className="bg-white p-6 rounded-[2.5rem] w-full max-w-lg max-h-[90vh] flex flex-col text-center border-2 border-slate-900 shadow-2xl relative overflow-hidden"
        onClick={e => e.stopPropagation()} // Prevent closing when clicking inside the modal content
      >
        <div className="absolute top-0 right-0 z-10 p-4"> {/* Standardized padding */}
            <button 
                onClick={onClose}
        ...
        <div className="pt-8 flex flex-col h-full"> {/* Added padding top for header space */}
            {title && <h3 className="text-2xl font-black mb-2 text-slate-900" dangerouslySetInnerHTML={{ __html: title }}></h3>}
